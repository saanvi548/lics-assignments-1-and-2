...
int main() {
    ...
    active_curr[S] = 1; // source starts active

    for (int hop = 0; hop < K; hop++) 
    {
        // clear next-hop arrays
        for (int i = 0; i < N; i++) 
        {
            active_next[i] = 0;
            for (int j = 0; j < N; j++) 
        ...
        for (int curr = 0; curr < N; curr++) 
        {
            if (active_curr[curr]) {
                for (int neigh = 0; neigh < N; neigh++) 
                {
                    if (grid[curr][neigh]) 
                    {
                        // don't send back to the tower that sent you
                        if (!parents_curr[curr][neigh]) 
                        {
                            active_next[neigh] = 1;
                            parents_next[neigh][curr] = 1;
                        }
                    }
                }
            }
